...
class EnhancedWorker(Worker):
    def __init__(self, *args, **kwargs):
        ...
        self.custom_job_start_time = (
            None  # Add custom to prevent interference with base class
        )
    ...
    def heartbeat(self, *args, **kwargs):
        ...
        if (
            current_time - self.custom_last_heartbeat
            > self.custom_health_check_interval
        ):
            # Log health status
            memory_usage = (
...
try:
    ...
    def handle_job_failure(job, *args, **kwargs):
        ...
        error_type = (
            kwargs.get("exc_type").__name__ if kwargs.get("exc_type") else "Unknown"
        )
        error_value = (
            str(kwargs.get("exc_value")) if kwargs.get("exc_value") else "No details"
        )
        ...
        error_context = {
            ...
            "job_duration": time.time() - worker.custom_job_start_time
            if worker.custom_job_start_time
            else None,
    ...
    def handle_job_success(job, queue, started_job_registry):
        ...
        logger.info(
            f"Worker {worker_name}: Job {job.id} completed", extra=success_context
        )
