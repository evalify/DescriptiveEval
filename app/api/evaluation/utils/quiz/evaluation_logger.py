...
class EvaluationLogger:
    ...
    def _validate_answer_format(self, question_type: str, answer: Any) -> bool:
        ...
        if question_type == "MCQ":
            return isinstance(answer, list) and all(
                isinstance(opt, str) for opt in answer
            )
        elif question_type == "TRUE_FALSE":
            return (
    ...
    def log_question_evaluation(
        ...
        student_answer = self._format_student_answer(
            response_data.get("student_answer"), question_type
        )
        ...
        if question_type == "MCQ":
            ...
            for response in responses.values():
                for option in response.get("selectedOptions", []):
                    stats["optionDistribution"][option] = (
                        stats["optionDistribution"].get(option, 0) + 1
                    )
        elif question_type == "CODING":
    ...
    def _save_log(self) -> None:
        ...
        with open(self.log_file, "w") as f:
            json.dump(self.evaluation_data, f, indent=2, cls=DateTimeEncoder)
