document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.result').forEach(result => {
        if (!result.textContent.trim()) {
            result.innerHTML = '<div class="empty-result">Results will appear here...</div>';
        }
    });
    ...
    const updateThemeButtonText = () => {
        ...
        themeToggle.innerHTML = `${icon} ${currentTheme === 'light' ? 'Dark' : 'Light'} Mode`;
    };
...
`;
    ...
    async function callAPI(endpoint, data) {
        ...
        try {
            ...
            const formatter = formatters[endpoint] || (data => JSON.stringify(data, null, 2));
            const markdown = formatter(json);
            result.innerHTML = marked.parse(markdown);
        } catch (error) {
            result.innerHTML = marked.parse(`# Error\n\`\`\`\n${error.message}\n\`\`\``);
        } finally {
    ...
    document.querySelector('#guidelines-form').addEventListener('submit', async (e) => {
        e.preventDefault();
    ...
    document.querySelector('#enhance-form').addEventListener('submit', async (e) => {
        e.preventDefault();
    ...
    document.querySelector('#evaluate-form').addEventListener('submit', async (e) => {
        e.preventDefault();
