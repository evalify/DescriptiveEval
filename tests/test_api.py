...
async def test_switch_to_groq(client):
    response = await client.post("/set-provider", json={"provider": "groq"})
...
async def test_switch_to_ollama(client):
    response = await client.post("/set-provider", json={"provider": "ollama"})
    assert response.status_code == 200
...
async def test_scoring_endpoint(client, sample_answer):
    ...
    log_evaluation("API Scoring Endpoint", {"request": sample_answer, "status_code": response.status_code}, result)
...
async def test_scoring_empty_answer(client, sample_answer):
    ...
    response = await client.post("/score", json=sample_answer)
    result = response.json()
    log_evaluation("API Empty Answer", {"request": sample_answer, "status_code": response.status_code}, result)
...
async def test_scoring_with_guidelines(client, sample_answer):
    ...
    response = await client.post("/score", json=sample_answer)
    result = response.json()
    log_evaluation("API Scoring With Guidelines", {"request": sample_answer, "status_code": response.status_code},
        result)
...
async def test_scoring_with_empty_guidelines(client, sample_answer):
    ...
    response = await client.post("/score", json=sample_answer)
    result = response.json()
    log_evaluation("API Scoring With Empty Guidelines", {"request": sample_answer, "status_code": response.status_code},
        result)
...
async def test_concurrent_vs_sequential_scoring(client, sample_answer):
    ...
    queries = [{**sample_answer, "question": "What is photosynthesis?",
        ...
        {**sample_answer, "question": "What is cellular respiration?",
        ...
        {**sample_answer, "question": "What is mitosis?",
        ...
        {**sample_answer, "question": "What is meiosis?",
    ...
    start_time = time.time()
    concurrent_responses = await asyncio.gather(*[client.post("/score", json=query) for query in queries])
    concurrent_time = time.time() - start_time

    # Log results
    performance_data = {"sequential_time": sequential_time, "concurrent_time": concurrent_time,
...
async def test_generate_guidelines_endpoint(client: AsyncClient):
    ...
    request_data = {"question": "Explain the process of photosynthesis.",
...
async def test_enhance_qa_endpoint(client: AsyncClient):
    ...
    request_data = {"question": "What is photosynthesis?", "expected_ans": "Process where plants make food"}
...
async def test_evaluate_endpoint(client: AsyncClient):
    ...
    request_data = {"quiz_id": "cm64n3edl0006xyrxnp4llbe4"}
...
async def test_score_endpoint(client: AsyncClient):
    ...
    request_data = {"question": "Explain photosynthesis.", "student_ans": "Plants convert sunlight to energy.",
        "expected_ans": "Photosynthesis is the process where plants convert sunlight into chemical energy.",
        "total_score": 10, "guidelines": "Focus on accuracy and completeness."}
